...
public class ArtifactService {
    ...
    public String download(List<String> ids){
        ...
        for(String id: ids){
            ThreadPoolUtils.execute(() -> {
                ArtifactService.downLoadOne(id);
            });
        }
    ...
    //单文件下载
    private static String downLoadOne(String id) {
        //根据id获取URL
    ...
    public static void downloadFileWithResume(String fileURL, String saveFilePath, Proxy proxy) throws IOException {
        File file = new File(saveFilePath);
        File progressFile = new File(saveFilePath + ".progress"); // 进度文件
        ...
        if (file.exists()) {
            ...
            if (remoteFileSize == file.length()) {
                System.out.println("该文件已存在且完整，无需重新下载");
                return; // 文件已存在且完整，直接返回
            } else {
                System.out.println("文件已存在但不完整，继续下载");
        ...
        if (progressFile.exists()) {
            try (FileInputStream fis = new FileInputStream(progressFile)) {
                ...
                System.out.println("检测到未完成的下载，继续从 " + formatFileSize(existingFileSize) + " 开始下载");
            }
        }
        ...
        try {
            ...
            connection.connect();
            System.out.println("直接连接成功，无需使用代理");
        } catch (IOException e) {
            System.out.println("直接连接失败，尝试使用代理...");
            useProxy = true;
        }
        ...
        System.out.println("文件总大小: " + formatFileSize(fileSize));

        // 检查服务器是否支持断点续传
        if (connection.getResponseCode() != HttpURLConnection.HTTP_PARTIAL) {
            System.out.println("服务器不支持断点续传，从头开始下载");
            existingFileSize = 0;
        ...
        try (InputStream inputStream = connection.getInputStream();
            ...
            while ((bytesRead = inputStream.read(buffer)) != -1) {
                ...
                System.out.printf("下载进度: %.2f%% (%s/%s)%n",
                        progress,
     ...
     */
    private static HttpURLConnection createConnection(String fileURL, Proxy proxy, long existingFileSize) throws IOException {
     ...
     * 获取远程文件的大小
     */
    private static long getRemoteFileSize(String fileURL, Proxy proxy) throws IOException {
        URL url = new URL(fileURL);
        HttpURLConnection connection;
        if (proxy != null) {
            connection = (HttpURLConnection) url.openConnection(proxy);
     ...
     * 更新进度文件
     */
    private static void updateProgressFile(File progressFile, long totalBytesRead) throws IOException {
